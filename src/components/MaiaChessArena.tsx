...
import { getFixedOpeningMove, getFixedWhiteOpeningMove, FixedOpening } from '../constants/chessOpenings';
import {
  MaiaChessDifficulty,
...
export function MaiaChessArena() {
  ...
  const resetGame = (
    nextDifficulty: MaiaChessDifficulty = difficulty,
    ...
    if (unlocked[nextDifficulty]) {
      setStatusLabel(
        `You are playing ${color === 'w' ? 'white' : 'black'} vs Maia AI (${difficultyMeta[nextDifficulty].elo} Elo).`,
      );
      // If the player chose black, Maia plays white and moves first.
      if (color === 'b') {
        triggerMaiaMove([], 'w');
      }
    } else {
  ...
  const handleSquarePress = (square: string) => {
    ...
    if (selectedSquare && legalTargets.includes(square)) {
      ...
      if (move) {
        ...
        if (!finished) {
          triggerMaiaMove(newHistory, playerColor === 'w' ? 'b' : 'w'); // Pass updated history
        }
      }
    }
  };

  const triggerMaiaMove = (currentHistory: string[] = moveHistory, aiColor: 'w' | 'b' = 'b') => {
    if (aiMoveTimeout.current) {
      clearTimeout(aiMoveTimeout.current);
    }
    setIsThinking(true);
    ...
    aiMoveTimeout.current = setTimeout(async () => {
      const game = chessRef.current;
      try {
        // Use a fixed opening book for Maia's first 2 moves (separate books for
        // White and Black) to add variety and avoid repetitive early predictions.
        const { move: openingMove, opening: newOpening } =
          aiColor === 'b'
            ? getFixedOpeningMove(currentHistory, selectedOpening)
            : getFixedWhiteOpeningMove(currentHistory, selectedOpening);
